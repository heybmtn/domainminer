import { phraseFromLabel } from "./vocab.mjs";
...
/** UK Ads phrase: hyphens become spaces; glued labels split into dictionary words. */
export function keywordFromDomain(input) {
  const d = normalizeDomain(input) || String(input || "").trim().toLowerCase();
  if (!d) return "";
  let label = d;
  if (label.endsWith(".co.uk")) label = label.slice(0, -6);
  else if (label.endsWith(".uk")) label = label.slice(0, -3);
  else {
    const dot = label.lastIndexOf(".");
    if (dot > 0) label = label.slice(0, dot);
  }
  return phraseFromLabel(label);
}
