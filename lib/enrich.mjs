...
export function missingSearchVolume(row) {
  return !row || row.search_volume == null || row.search_volume === "";
}
...
export async function enrichDomains(rawDomains, {
  ...
  for (const d of domains) {
    if (!force && cachedMap[d]) {
      ...
      const needVolume = missingSearchVolume(cachedMap[d]);
  ...
  const items = domains.map((d) => {
    const fullCache = Boolean(
      !force && cachedMap[d] && !missingOrganicCount(cachedMap[d]) && !missingSearchVolume(cachedMap[d]),
    );
