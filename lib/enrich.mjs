...
const BULK = {
  ...
  trend: "/v3/backlinks/bulk_new_lost_referring_domains/live",
};
...
export async function enrichDomains(rawDomains, {
  ...
  if (cache) await recordSpend({ cache, service: "dataforseo", amount: cost });
  const spend = await getSpend({ cache, service: "dataforseo" });
...
/**
 * Second-pass link-trend check for domains that already cleared "buy" — not
 * run automatically by enrichDomains, since it's an extra paid call meant to
 * be spent only on names you're about to actually register. A domain whose
 * referring domains are mostly gained-then-lost is a different buy than one
 * that's been stable, even with the same point-in-time Ref count.
 */
export async function verifyDomains(rawDomains, { cache, client, budgetCapUSD = null } = {}) {
  const seen = new Set();
  const domains = [];
  for (const raw of rawDomains || []) {
    const d = normalizeDomain(raw);
  ...
  const json = await client.post(BULK.trend, { targets: domains });
  const parsed = itemsFromTask(json);
  const trendMap = indexByTarget(parsed.items);
  const items = domains.map((d) => {
    const t = trendMap.get(d) || {};
    return {
      domain: d,
      newReferringDomains: num(t.new_referring_domains),
      lostReferringDomains: num(t.lost_referring_domains),
    };
  });
  ...
  return { items, cost, spendTotal: spend.total };
}
