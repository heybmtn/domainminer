...
export function hasTraffic({ etv = 0, search_volume = 0 } = {}) {
  return (Number(etv) || 0) > 0 || (Number(search_volume) || 0) > 0;
}
...
export function brandBand(score) {
  ...
  if (!Number.isFinite(n)) return { key: "", label: "" };
  if (n >= 7) return { key: "strong", label: "Strong" };
...
export function buyVerdict(row = {}) {
  ...
  if (!links && !traffic) {
    return {
      ...
      verdictRank: 2,
      spam_band: band.key,
      reason: "No meaningful Rank, Ref, UK traffic, or search volume. Skip — nothing to buy for SEO.",
    };
  }
  if (spam >= 31) {
    return {
  ...
  if (links && traffic) {
    return {
      ...
      verdictRank: 0,
      spam_band: band.key,
      reason: "Low spam with Rank/Ref and UK traffic or search volume. Buy.",
    };
  }
  if (traffic) {
    ...
    return {
      ...
      reason: etv
        ...
        : "Low spam with UK search volume. Buy.",
    };
  }
  return {
    verdict: "buy",
