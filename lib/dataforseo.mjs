...
/** Google Ads search_volume returns result[] as keyword rows, not result[0].items. */
export function itemsFromKeywordResults(json) {
  const task = json?.tasks?.[0];
  const result = task?.result;
  let items = [];
  if (Array.isArray(result)) {
    if (result[0] && Array.isArray(result[0].items)) items = result[0].items;
    else items = result;
  }
  return {
    items,
...
export function indexByKeyword(items) {
  const map = new Map();
  for (const item of items || []) {
