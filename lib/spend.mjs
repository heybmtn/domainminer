...
function spendKey(date) {
  return `spend-${monthId(date)}`;
}

/** Adds a completed call's cost to this UTC month's running total. No-ops for a zero/negative amount or no cache. */
export async function recordSpend({ cache, amount, now = () => new Date() } = {}) {
...
/** Current UTC month's running DataForSEO spend total. */
export async function getSpend({ cache, now = () => new Date() } = {}) {
  const date = now();
...
/** capUSD is optional (from DATAFORSEO_MONTHLY_BUDGET); null/unset means no cap enforced. */
